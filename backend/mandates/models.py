...
class PropertyOwner(models.Model):
    ...
    national_id_number = models.CharField(
        max_length=80,
        blank=True,
        help_text="Private. Never expose through public APIs.",
    )
    ...
    created_by = models.ForeignKey(
        ...
        related_name="created_property_owners",
    )
...
class PropertyMandate(models.Model):
    ...
    partner = models.ForeignKey(
        "partners.Partner",
        on_delete=models.PROTECT,
        related_name="property_mandates",
    )
    ...
    mandate_number = models.CharField(
        max_length=40,
        unique=True,
        blank=True,
        editable=False,
    ...
    owner_signed_date = models.DateField(
        null=True,
        blank=True,
    )
    ...
    no_cash_acknowledged = models.BooleanField(
        default=False,
        help_text=(
            "Owner acknowledged that cash and direct off-platform "
    ...
    anti_circumvention_acknowledged = models.BooleanField(
        default=False,
        help_text=(
            "Owner acknowledged that Pata Hao introductions "
            "cannot be bypassed."
        ),
    )
    ...
    owner_authority_confirmed = models.BooleanField(
        default=False,
        help_text=(
            "Owner or authorized representative confirmed authority "
            "to mandate this property."
        ),
    )
    ...
    created_by = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.PROTECT,
        related_name="created_property_mandates",
    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["property", "version"],
                name="unique_mandate_version_per_property",
            ),
        ]
    ...
    def clean(self):
        ...
        if self.property_id and self.partner_id:
            if self.property.partner_id != self.partner_id:
                errors["partner"] = (
                    "The mandate partner must match the property partner."
                )

        if (
            self.commission_agreement_id
            and self.property_id
            and self.commission_agreement.property_id
            != self.property_id
        ):
            errors["commission_agreement"] = (
                "The commission agreement must belong to this property."
            )
        ...
        if self.status == self.Status.APPROVED:
            if not self.owner.is_verified:
                errors["owner"] = (
                    "The property owner must be verified before approval."
                )

            if not self.owner_authority_confirmed:
                errors["owner_authority_confirmed"] = (
                    "Owner authority must be confirmed."
                )

            if not self.no_cash_acknowledged:
                errors["no_cash_acknowledged"] = (
                    "The no-cash policy must be acknowledged."
                )

            if not self.anti_circumvention_acknowledged:
                errors["anti_circumvention_acknowledged"] = (
                    "The anti-circumvention rule must be acknowledged."
                )
            ...
            if not self.has_approved_signed_document:
                errors["status"] = (
                    "An approved signed mandate document is required."
                )

            if not self.commission_agreement.is_publish_ready():
                errors["commission_agreement"] = (
                    "The commission agreement must be confirmed, "
                    "verified, and locked."
                )
    ...
    def is_currently_valid(self):
        ...
        return (
            ...
            and self.owner_authority_confirmed
            and self.no_cash_acknowledged
            and self.anti_circumvention_acknowledged
            and self.has_approved_signed_document
            and self.commission_agreement.is_publish_ready()
        )

    def approve(self, *, approved_by):
        if approved_by is None or not approved_by.is_staff:
            raise ValidationError(
                "Only a Pata Hao administrator can approve a mandate."
            )
...
class MandateDocument(models.Model):
    ...
    uploaded_by = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.PROTECT,
        related_name="uploaded_mandate_documents",
    )

    reviewed_by = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.SET_NULL,
        related_name="reviewed_mandate_documents",
        null=True,
        blank=True,
    )
    ...
    def clean(self):
        ...
        if self.status == self.Status.APPROVED:
            if self.reviewed_by is None:
                errors["reviewed_by"] = (
                    "The reviewing administrator is required."
                )

            if self.reviewed_at is None:
                errors["reviewed_at"] = (
                    "The review date and time are required."
                )
