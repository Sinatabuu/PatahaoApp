...
class CommissionAgreement(models.Model):
    """
    Records the commission agreement between a property owner
    and Pata Hao before a property can be published.

    A property is publish-ready only when this agreement has been:

    1. Confirmed by the owner.
    2. Verified by an administrator.
    3. Locked against financial changes.
    """
    ...
    class Status(models.TextChoices):
        ...
        OWNER_CONFIRMED = "owner_confirmed", "Owner confirmed"
    ...
    transaction_value = models.DecimalField(
        ...
        decimal_places=2,
        help_text=(
            "The expected rent or sale value used to calculate "
            "the commission."
        ),
    )
    ...
    def clean(self):
        ...
        if self.commission_method == self.CommissionMethod.PERCENTAGE:
            if self.commission_rate is None:
                errors["commission_rate"] = (
                    "A commission rate is required for a "
            ...
            if self.fixed_commission_amount is not None:
                errors["fixed_commission_amount"] = (
                    "Do not enter a fixed amount for a "
                    "percentage agreement."
                )
            ...
            if self.fixed_commission_amount is None:
                errors["fixed_commission_amount"] = (
                    "A fixed commission amount is required for "
                    "a fixed agreement."
                )
            elif self.fixed_commission_amount <= Decimal("0.00"):
                errors["fixed_commission_amount"] = (
                    "The fixed commission amount must be "
                    "greater than zero."
                )

            if self.commission_rate is not None:
                errors["commission_rate"] = (
                    "Do not enter a commission rate for a "
                    "fixed agreement."
                )
        ...
        if self.is_verified:
            if not self.owner_confirmed:
                errors["is_verified"] = (
                    "The agreement cannot be verified before "
                    "owner confirmation."
                )

            if self.verified_by is None:
                errors["verified_by"] = (
                    "The administrator who verified the agreement "
                    "is required."
                )
            ...
            if self.verified_at is not None:
                errors["verified_at"] = (
                    "A verification time cannot be recorded "
                    "before verification."
                )
        ...
        if self.is_locked:
            ...
            errors["locked_at"] = (
                "A locking time cannot be recorded before "
                "the agreement is locked."
            )

        if self.status == self.Status.OWNER_CONFIRMED:
            if not self.owner_confirmed:
                errors["status"] = (
                    "Owner-confirmed status requires owner confirmation."
                )
    ...
    def calculate_expected_total(self):
        """
        Calculates the complete commission owed by the property owner.

        This is not the individual partner's earning. Partner earnings
        will be calculated separately from the applicable commission plan.
        """

        if self.commission_method == self.CommissionMethod.FIXED:
            amount = self.fixed_commission_amount or Decimal("0.00")
    ...
    def is_publish_ready(self):
        ...
        """
        Indicates whether this agreement satisfies the financial
        requirements for publishing its property.
        """
    ...
    def verify(self, verified_by):
        ...
        if not self.owner_confirmed:
            raise ValidationError(
                "Owner confirmation is required before verification."
            )

        if verified_by is None:
            raise ValidationError(
                "The verifying administrator is required."
            )
    ...
    def _validate_locked_fields(self):
        """
        Prevents financial and ownership details from being modified
        after an agreement has been locked.
        """

        if not self.pk:
            return

        original = CommissionAgreement.objects.get(pk=self.pk)
        ...
        for field_name in protected_fields:
            ...
            if old_value != new_value:
                raise ValidationError(
                    {
                        field_name: (
                            "This field cannot be changed because the "
